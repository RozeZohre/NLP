...
from org.apache.lucene.search.similarities import ClassicSimilarity  # Change here
...
try:
    ...
    with closing(DirectoryReader.open(FSDirectory.open(index_path))) as reader:
        # Use ClassicSimilarity instead of BM25Similarity for TF-IDF
        searcher = IndexSearcher(reader)
        searcher.setSimilarity(ClassicSimilarity())  # Change here
            ...
            for filename in os.listdir(topics_path):
                if filename.endswith(".txt"):
                    ...
                    try:
                        ...
                        # Use regex to extract the content inside the <num>, <dom>, and <desc> tags
                        num_match = re.search(r'<num> Number:\s*([^\n]+)', content)
                        domain_match = re.search(r'<dom> Domain:\s*([^\n]+)', content)
                        desc_match = re.search(r'<desc> Description:\s*([^<]+)', content)

                        if num_match and domain_match and desc_match:
                            num_info = num_match.group(1).strip()
                            domain_info = domain_match.group(1).strip()
                            desc_info = desc_match.group(1).strip()

                            # Combine num, desc, and domain info
                            query_string = f"{int(num_info)} {domain_info} {QueryParser.escape(desc_info)}"
